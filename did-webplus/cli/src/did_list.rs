...
pub struct DIDList {
    ...
    pub self_hash_o: Option<mbc::MBHash>,
...
fn parse_mbhash_from_string(s: &str) -> Result<mbc::MBHash> {
    mbc::MBHash::try_from(s)
    ...
    pub async fn handle(self) -> Result<()> {
        ...
        for did_doc_record in did_doc_record_v.into_iter() {
            ...
            let did_string = if self.fully_qualified {
                let query_self_hash = mbc::MBHashStr::new_ref(&did_doc_record.self_hash).unwrap();
