...
impl Wallet {
    ...
    pub async fn create_did(&self, vdr_did_create_endpoint: String) -> Result<String, JsValue> {
    ...
    pub async fn fetch_did(&self, did: String, vdr_scheme: String) -> Result<(), JsValue> {
    ...
    pub async fn update_did(&self, did: String, vdr_scheme: String) -> Result<String, JsValue> {
        let did = did_webplus_core::DIDStr::new_ref(&did).map_err(into_js_value)?;
        let vdr_scheme = Self::parse_http_scheme(vdr_scheme.as_str())?;
        let controlled_did = self
            .deref()
